...
ps.final.r.ProteoClAllPhy.plot<-ggplot(ps.final.r.ProteoClAllPhy, 
...
ps.final.r.ProteoClAllPhy.plot
```

## 3b - Phyla- and Proteobacterial class-level stacked barchart by DNA_or_cDNA~Snowpack_Layer

```{r}
ps.final.r.ProteoClAllPhy.plot_layer<-ggplot(ps.final.r.ProteoClAllPhy, 
                                   aes(x = Month, y = Abundance, fill = Taxa)) + 
  ...
  guides(fill = guide_legend(keywidth = 1, keyheight = 1)) +
  ylab("Relative Abundance\n")
ps.final.r.ProteoClAllPhy.plot_layer
```
...
ps.final.r.glom.genus.psdf.plot<-ggplot(ps.final.r.glom.genus.psdf, 
  ...
  guides(fill = guide_legend(keywidth = 1, keyheight = 1)) +
  ylab("Relative Abundance\n")
ps.final.r.glom.genus.psdf.plot
```
...
```{r eval=FALSE, include=FALSE}
plot_grid(ps.final.r.dna.nmds.plot, ps.final.r.cdna.nmds.plot)
```
```{r eval=FALSE, include=FALSE}
...
slayer_comparisons = list( c("Top", "Mid"), c("Top", "Basal"), c("Top", "Sup. Ice"), 
                           ...
                           c("Mid", "Glacial Ice"),
                           c("Basal", "Sup. Ice"), c("Basal", "Glacial Ice"),
                           c("Sup. Ice", "Glacial Ice"))
```
...
Rarefying to the smallest number of reads in any sample.
